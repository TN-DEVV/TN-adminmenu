...
local function openfirstmenu(CurrentPlayers,CurrentStaff)
    ...
    local elements = {
            ...
            select = function()
                QBCore.Functions.TriggerCallback('qb-adminmenu:callback:isheonduty', function(result)
                    if result == true then
                        ...
                        QBCore.Functions.Notify('tu doit etre on duty','error')
                    end
                end)
            end
        },
        [2] = {
            ...
            select = function()
                TriggerEvent('philip-adminduty:AdminDuty')
            end
        },
        [3] = {
...
RegisterNetEvent('qb-admin:client:maxmodVehicle', function()
    ...
    QBCore.Functions.Notify("your vehicle is full upgrade right now","success")
end)
